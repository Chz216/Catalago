...
class funciones {
    ...
    foreach ($conn as $navbar) {
      ...
      echo"
        <li class='nv-item i-down'>
          <a id='$id_navbar' class='nv-link' href='$href'>$nom_navbar</a>
      ";
      $con = $this->bd->navbar_nav($id_navbar);
      if ($con) {
        echo "<ul class='sub-nav'>";
        foreach ($con as $nav) {
          $id_nav = $nav["Id_nav"];
          $nav = $nav["Nom_category"];
  ...
  public function showProducts(){
        $id = $_POST['id_navbar'];
        $conn = $this->bd->products($id);
        $productos = '';
        foreach ($conn as $prod) {
          $productos.='
          <li class="item-prod">
            <div class="img-prod">
            ...
            <div class="wrapper-prod">
              <div class="name-shoe">
                <span>'.$prod['Nom_producto'].'</span>
              </div>
            <div class="price-shoe">
              <span>$'.$prod['Precio'].'.00</span>
            </div>
          <div class="ide-shoe">
          <a id="active" href="detalles.php?modelo='.$prod['Id_producto'].'">Ver Detalles</a>
          </div>
          </div>
          </li>
          ';
